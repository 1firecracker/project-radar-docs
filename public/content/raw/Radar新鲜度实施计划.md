...
- [x] 先添加失败测试：`pending`、`snoozed` 可以转为 `superseded`，`accepted/running` 不受影响。
- [x] 添加失败测试：`supersede_pending_radar_items_for_session(user_id, session_id)` 只修改指定用户和 Session，并返回修改数量。
- [x] 运行定向测试，确认因缺少状态和函数而失败。
- [x] 在 `_RADAR_TRANSITIONS` 中增加：
...
- [x] 先添加失败测试：Session 为 `running/waiting_user_input` 时旧 Event 不可生成 Radar。
- [x] 添加失败测试：Session 回到 `idle` 后，只有最新 `text/upload_only` User Record 对应的 Event 可生成 Radar；较旧 Event 返回 stale。
- [x] 实现：
...
- [x] 在 `generate_post_run_radar` 调用 Analyzer 前检查一次，避免浪费模型调用；Analyzer 返回后、保存前再检查一次，收敛并发窗口。
- [x] stale 时返回 `{"outcome": "stale", "radar": None}`，Organizer 结果保持已应用。
- [x] 重跑数据库与 Radar Service 定向测试。
...
- [x] 先添加失败测试：普通真实 Query 成功 claim Conversation 后，指定 Session 的 `pending/snoozed` Radar 变为 `superseded`。
- [x] 添加边界测试：Radar Action Run 和 Resume 不会把已 `accepted/running` 的建议淘汰。
- [x] 在 `_prepare_agent_start` 中，仅对没有 `radar_item_id` 的普通 Query 淘汰；Conversation claim 与状态更新位于同一事务。
- [x] 若后续上传处理失败，不恢复旧 Radar；用户已经开始新真实轮次，旧建议仍视为过期。
- [x] 重跑 Query 准备、执行与数据库定向测试。
...
- [x] 先添加失败测试：Radar Analyzer 上下文包含 `latest_qa.user_text` 与 `latest_qa.final_assistant_text`。
- [x] 添加 Prompt 测试：若最终 Assistant 已指出同一缺口、风险或下一步，Analyzer 必须返回空对象，不得通过改写标题重复提醒。
- [x] `run_post_run_job` 把 payload 中的原始用户文本与最终 Assistant 文本传给 `generate_post_run_radar`。
- [x] `generate_post_run_radar` 将两段文本加入 Analyzer context。
- [x] 修改 `_RADAR_INSTRUCTION`，明确：
