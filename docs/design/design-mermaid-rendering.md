...
图表成功渲染后，`MermaidBlock` 显示右上角工具栏。点击“全屏”后，组件使用页面内固定定位覆盖整个视口，不调用浏览器 Fullscreen API，因此不触发权限提示。全屏容器使用具名 modal dialog 语义，键盘焦点限制在退出按钮，并在退出时恢复。页面背景滚动由页面级引用计数锁管理，多个图表状态交错时也只在最后一个全屏结束后恢复原始 inline overflow；图表区域始终保留双向滚动。点击“退出全屏”或按 `Esc` 均恢复原位。组件卸载时必须清除键盘监听、释放共享滚动锁并恢复焦点。加载中或渲染失败时不显示全屏按钮。
...
- 组件测试：两个图表交错进入和退出全屏时共享滚动锁，最后一个退出后精确恢复原值。
- 组件测试：全屏容器提供 modal dialog 语义，`Tab` 不会进入被遮挡页面，退出后恢复焦点。
